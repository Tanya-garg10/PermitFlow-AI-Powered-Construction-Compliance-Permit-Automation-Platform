...
export default function ProjectWizard({ onBack, onSubmit, isSubmitting }: ProjectWizardProps) {
  // Form States
  ...
  const handleVoiceInput = () => {
    setIsRecording(true);
    setVoiceLog('Listening for speech... Speak your plot parameters...');
    
    setTimeout(() => {
      setVoiceLog('Transcribing: "My plot area is 400 square meters. Building height is 7.5 meters with 2 floors of residential use."');
      ...
      setTimeout(() => {
        ...
        setVoiceLog('Forms pre-filled successfully from voice input!');
      }, 1500);
  ...
  const handleFormSubmit = (e: React.FormEvent) => {
    ...
    if (!name || !location || !plotArea || !floors || !height) {
      alert("Please fill in all details or use the Voice Input feature!");
      return;
    }

    setRunOcr(true);
    setOcrProgress(10);
    setOcrSteps(["Initializing Tesseract & PaddleOCR framework..."]);

    const steps = [
      "Extracting bounding coordinates from drawing board raster vectors...",
      "Extracting room labels and door coordinate offsets...",
      "Extracting occupancy indicators: detected single-family occupancy structure.",
      "Extracting height dimensions and structural deck clearances...",
      "Calculated plot bounding coefficient: Floor Area Ratio (FAR) matches.",
      "Triggering Sarvam Document Intelligence translation checks...",
      "Sending compiled vector blocks to server for zoning checks..."
    ];
  ...
  return (
    <div id="project-wizard-container" className="max-w-4xl mx-auto bg-white border border-slate-200 rounded-2xl p-6 md:p-8 shadow-sm text-slate-800 font-sans">
      ...
      {/* Title */}
      <div className="mb-8">
        <h2 className="text-xl md:text-2xl font-black tracking-tight text-slate-900">Project Blueprint Autopilot</h2>
        <p className="text-xs text-slate-500 mt-1.5 leading-relaxed">
          Upload blueprints and documents to initiate OCR parsing and Sarvam document intelligence auditing.
        </p>
      </div>
        ...
        <form onSubmit={handleFormSubmit} className="space-y-6">
          ...
          <div className="p-4 bg-slate-50 border border-slate-200 rounded-xl flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
            <div>
              <h4 className="text-xs font-mono text-blue-600 font-black uppercase flex items-center gap-1.5">
                <Volume2 className="w-4 h-4" /> Sarvam Speech Recognition
              </h4>
              <p className="text-[11px] text-slate-600 mt-1 leading-relaxed">
                Have building dimensions ready? Click record to speak. Sarvam AI will automatically structure, parse and fill the form fields for you.
              </p>
              {voiceLog && (
            ...
            >
              ...
              <span>{isRecording ? 'Listening...' : 'Voice Assist'}</span>
            </button>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-4">
              <span className="text-xs font-mono text-slate-400 font-bold uppercase tracking-wider block">Project Identity</span>
              
              <div>
                <label className="block text-xs font-semibold text-slate-700 mb-1">Project Portfolio Name</label>
                <input
                  ...
                  placeholder="e.g., Gurugram Residential Villa"
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-700 mb-1">Municipal Location Address</label>
                <input
                  ...
                  placeholder="e.g., Sector 15, Gurugram, HR"
                  className="w-full bg-white border border-slate-200 rounded-lg px-3 py-2 text-xs text-slate-800 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500"
                />
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-semibold text-slate-700 mb-1">Plot Area (sqm)</label>
                  <input
                ...
                <div>
                  <label className="block text-xs font-semibold text-slate-700 mb-1">Building Use Code</label>
                  <select
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-xs font-semibold text-slate-700 mb-1">Max Height (meters)</label>
                  <input
            ...
            <div className="space-y-4">
              <span className="text-xs font-mono text-slate-400 font-bold uppercase tracking-wider block">Required Document Attachments</span>
              
              <div>
                <label className="block text-xs font-semibold text-slate-700 mb-2">Vector Blueprint drawing (PDF / DWG / PNG)</label>
                <div
                ...
                >
                  ...
                  <p className="text-xs text-slate-700 font-bold">
                    {blueprintName ? blueprintName : "Drag & drop blueprint layout"}
                  </p>
                  <p className="text-[10px] text-slate-500 mt-1">Supports PDF, CAD file, image up to 20MB</p>
              ...
              <div className="bg-slate-50 p-4 rounded-xl border border-slate-200">
                <span className="text-[10px] font-mono text-slate-500 font-bold uppercase block mb-2.5">Checklist of standard enclosures</span>
                <div className="space-y-2 text-slate-700 text-[11px] font-medium">
                  <div className="flex items-center space-x-2">
                    ...
                    <span>Owner Property Registry Documents</span>
                  </div>
                  <div className="flex items-center space-x-2">
                    <input type="checkbox" defaultChecked className="rounded border-slate-300 bg-white accent-blue-600" />
                    <span>Registered Architect License Certificate</span>
                  </div>
                  <div className="flex items-center space-x-2">
                    <input type="checkbox" defaultChecked className="rounded border-slate-300 bg-white accent-blue-600" />
                    <span>Zoning NOC Application Form receipt</span>
                  </div>
                </div>
              </div>
